...
//   {
//     title: "Learning Edge",
//     content: "Empowering Growth, Innovation, Resilience",
//   },
//   {
//     title: "Consulting & Outsourcing",
//     content: "Unlocking Growth with Future-Ready IT Solutions",
//   },
//   {
//     title: "Products",
//     content: "Your Edge in a Rapidly Evolving Digital World",
//   },
// ];
...
//           ${
//             isMobile
//               ? "w-full px-4 pt-6 flex flex-col"
//               : "flex-col p-14 w-[26%] h-screen"
//           }
...
//       {activeIndex === 0 && <LearningEdge />}
//       {activeIndex === 1 && <Consulting />}
...
export default function ExpertiseScroll() {
  ...
  // 👇 Disable IntersectionObserver when on mobile
  useEffect(() => {
    if (isMobile) return;
  ...
  // 👇 Disable scroll-based section change when on mobile
  useEffect(() => {
    if (isMobile) return;
  ...
  return (
    <div
      ref={containerRef}
      className="relative flex flex-col md:flex-row  w-full bg-[#F6F6F9] rounded-t-4xl"
    >
      {/* Sidebar */}
      <div
        className={`${
          isMobile
            ? "w-full px-4 pt-6 flex flex-col"
            : "flex-col p-12 w-[26%] h-screen"
        } flex sticky top-10 bg-[#F6F6F9] z-30 rounded-t-4xl`}
      >
        <div className="w-full text-center mb-4 m-4">
          <p className="text-2xl md:text-3xl text-center md:text-start font-bold text-[#004781]">
            {isMobile ? (
              "Our Expertise"
            ) : (
        ...
        {isMobile ? (
          <div className="flex justify-center items-center w-full gap-2 overflow-x-auto pb-4">
            {sections.map((item, index) => (
              <button
                key={index}
                className={`flex-shrink-0 px-3 py-2 text-xs font-medium rounded-full transition-all whitespace-nowrap ${
                  activeIndex === index
                    ? "bg-black text-white"
                    : "bg-white text-gray-500 shadow-2xl rounded-2xl"
                }`}
          ...
          <>
            {sections.map((item, index) => (
              <div key={index} className="mb-5 transition-all w-full">
                ...
                >
                  <TextReveal>{item.content}</TextReveal>
                </div>
              </div>
            ))}
          </>
      ...
      {isMobile ? (
        <div className="w-full flex justify-center items-center py-2 px-4 transition-all duration-500 ease-in-out">
          {activeIndex === 0 && <LearningEdge />}
          {activeIndex === 1 && <Consulting />}
          {activeIndex === 2 && <Products />}
        </div>
